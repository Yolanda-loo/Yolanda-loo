Tebogo Mathaba 👨🏾‍💻


Aspiring Software Engineer · Data Analyst · Full-Stack Developer · Power BI Specialist
Crafting innovative tech solutions with a blend of creativity, technical expertise, and a passion for business management.

🧠 About Me

💡 Enthusiastic about building user-friendly web applications and data-driven solutions.
📊 Skilled in creating impactful dashboards and performing data analysis with Power BI.
🔍 Committed to mastering full-stack development with a focus on Java, Python, and JavaScript.
🌍 Dedicated to empowering communities through technology and leadership, with experience at Founder Institute and WeThinkCode.
🚀 Always learning — currently diving deeper into software engineering at WeThinkCode and exploring cloud technologies like Microsoft Azure.


🔧 Tech Toolbox

Languages: Python, JavaScript, HTML, CSS, C#, SQL
Tools & Frameworks: Power BI, Flask, React, Tailwind CSS, Azure
Skills: Data Analysis, Full-Stack Development, Testing, Project Management, Customer Service
Certifications: 
Microsoft Azure Developer Associate (AZ-204)
Azure Fundamentals (AZ-900)
Microsoft Digital Literacy
Web Designer Certification
Project Planning (Spec Con)
AWS APAC Solutions Architecture Job Simulation
FNB Full stack Developer




🚀 Featured Projects


🌟 Project
⚡ Description
🛠️ Tech Stack



Power BI Dashboards
Developed management dashboards for scheduling and business analysis at Volkswagen
Power BI, SQL


Personal Portfolio Website
A responsive portfolio showcasing my projects and skills
HTML, CSS, JavaScript, Tailwind CSS


Task Management App
A secure task manager with user authentication and a clean UI
Flask, PostgreSQL, JavaScript, Tailwind CSS


Kasi Business Platform
A web app to connect township businesses with investors for job creation
React, Tailwind CSS, Firebase


Hangman Game
An interactive game with a Tkinter GUI for fun and learning
Python, Tkinter



🧪 Currently Working On

Completing my final year at WeThinkCode to become a proficient software engineer.
Building a Progressive Web App for task management with offline capabilities.
Enhancing my skills in cloud computing with Azure and AWS certifications.
Contributing to community-driven tech projects to foster digital skill empowerment.


🌍 Community & Leadership

Founder Institute South Africa: Assistant Administrator, supporting tech entrepreneurs with strategic thinking.
WeThinkCode: Peer Mentor, helping peers with coding challenges and fostering collaborative learning.
Higher Health SA: Volunteer for health screenings, promoting community wellness.
Editor-in-Chief: Lead my school newspaper,learning leadership and communication skills.


📫 Connect with Me

Location: Johannesburg, Gauteng, South Africa
LinkedIn: Tebogo Mathaba
Email: tebogomathaba09@gmail.com
Portfolio: [Your Portfolio URL Here]


"From hesitation to celebration! Pushing through challenges fuels growth." — Tebogo Mathaba
